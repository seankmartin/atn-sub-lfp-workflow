...
def ripple_control(r, resting):
    ...
    lfp_data_sub = np.array(
        [lfp_data[i] for i in range(len(brain_regions)) if brain_regions[i] == "SUB"]
    ).T
    ...
    ratio_resting = np.sum(resting) / len(speed)
    return final_times, non_times, ratio_resting
